...
- MCP: `apps/api/src/mcp-server.ts` exposes every service method as an MCP tool over stdio (core's `McpRegistry`). One deliberate exception: `gameService` is not registered, because `McpRegistry.invoke()` carries no socket and every game method binds to one (live sim presence, `ctx.socketId` room grants) in a process that runs no sim loop. Root `.mcp.json` wires it for Claude Code; run `bun run build` first, then `bun run mcp` from `apps/api` to run it manually
